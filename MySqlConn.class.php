...
class MySqlConn
{
            ...
            !empty ($this->_password))   // to make sure they are set.
        {
            ...
            if ($this->_link == false) {
                $emsg = 'connect_to_database: mysql_connect returned false. No db connection.<br />'
                    . 'The MySQL Error is: ' . mysqli_error($this->_link);
    ...
    public function select_database($p_dbname = '', $p_dbflag = false)
    {
        ...
        $db_selected = mysqli_select_db($this->_link, $p_dbname);
        if ($db_selected)
            return true;
        else {
    ...
    public function execute_query($p_dbflag = false)
    {
        ...
        $this->_result_set = mysqli_query($this->_link, $this->_query_string);
        if ($this->_result_set) {
            if ($this->_debug_flag || $p_dbflag) {
                // These may product warnings depending on the type of query issued.
                // For debug mode, do them anyway
                $this->_num_returned_rows = mysqli_num_rows($this->_result_set);
                $this->_num_affected_rows = mysqli_affected_rows($this->_link);
    ...
    public function get_first_result($p_how = RETURN_ASSOC_ARRAY, $p_dbflag = false)
    {
        if ($this->_num_returned_rows == -1 || $this->_num_affected_rows == -1) {
            ...
            if ($this->_debug_flag || $p_dbflag) {
                ...
                $result = mysqli_data_seek($this->_result_set, 0);
            } else {
                @$result = mysqli_data_seek($this->_result_set, 0);
            }
            if ($result) {
    ...
    private function _get_result($p_how = RETURN_ASSOC_ARRAY, $p_dbflag = false)
    {
        ...
        switch ($p_how) {
            case RETURN_NUM_ARRAY:
                $result = mysqli_fetch_array($this->_result_set, MYSQLI_NUM);
                if ($result)
                    $this->_row = $result;
                else
                    $this->_row = null;
            ...
            case RETURN_ASSOC_ARRAY:
                $result = mysqli_fetch_array($this->_result_set, MYSQLI_ASSOC);
                if ($result)
                    $this->_row = $result;
                else
                    $this->_row = null;
            ...
            case RETURN_OBJECT:
                $result = mysqli_fetch_object($this->_result_set);
                if ($result)
                    $this->_row = $result;
                else
                    $this->_row = null;
